...
static void USB_RecvTask(void *param)
{
    ...
    while (1)
    {
        ...
        if(trans->pack_id)
        {
            if(last_pack_id<=trans->pack_id)    //如果当前包的ID大于之前包的ID，说明此时是全新的传输任务，重新设置缓冲区
                buffer_index=0;
            last_pack_id=trans->pack_id;
            memcpy(_UserRxBufferFS+buffer_index,UserRxBufferFS,current_cdc_pack_size-sizeof(uint32_t));     //将接收的数据包拷贝到用户缓冲区
        }
        else
        {
